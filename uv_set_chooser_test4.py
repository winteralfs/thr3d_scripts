                ...
                if number_of_uv_sets > 0:
                    ...
                    while it <= number_of_uv_sets_for_object:
                        ...
                        for uv_set in uv_sets:
                            ...
                            i = i + 1
                        it = it + 1
        self.uv_sets_with_space_list = []
        self.uv_sets_with_space_dic = {}
        for uv_set in self.uv_sets_all:
            if ' ' in uv_set and '*' not in uv_set and uv_set != '  ':
                print 'uv_set = ',uv_set
                uv_set_split = uv_set.split(':|:')
                print 'uv_set_split = ',uv_set_split
                uv_set = uv_set_split[1]
                print 'uv_set = ',uv_set
                self.uv_sets_with_space_list.append(uv_set)
                uv_set_name_no_space = uv_set.replace(' ','')
                self.uv_sets_with_space_dic[uv_set_name_no_space] = uv_set
                    ...
                    if item_uv_set_text_split[0] != '':
                        if i == selected_row:
                            if item_uv_set_text == self.selected_item_text:
                                #self.selected_item_text =  self.selected_item_text.replace(' ','')
                                #modified_name = 0
                                #if self.selected_item_text in self.uv_sets_with_space_dic:
                                    #self.selected_item_text = self.uv_sets_with_space_dic[self.selected_item_text]
                                    #modified_name = 1
                                item_text_selection_status_dic_key = (item_text + ':|:' + (item_object  + ':|:' + self.selected_item_text))
                                #if modified_name == 1:
                                    #for self.uv_set_with_space_dic in self.uv_sets_with_space_dic:
                                        #uv_set = self.uv_sets_with_space_dic[self.uv_set_with_space_dic]
                                        #self.selected_item_text = uv_set
    ...
    def link_texture_to_uv_set(self):
        ...
        for uv_set_selection in self.uv_set_selection_status_dic:
            ...
            if selection_status == 0:
                ...
                uv_set = uv_set_selection_split[2]
                if uv_set in self.uv_sets_with_space_dic:
                    uv_set = self.uv_sets_with_space_dic[uv_set]
            ...
            if selection_status == 1:
                ...
                uv_set = uv_set_selection_split[2]
                print 'uv_set = ',uv_set
                print 'self.uv_sets_with_space_dic = ',self.uv_sets_with_space_dic
                if uv_set in self.uv_sets_with_space_dic:
                    uv_set = self.uv_sets_with_space_dic[uv_set]
                print 'uv_set = ',uv_set
                texture_linked_uv_set_address = self.uv_set_name_to_address_dic[object + ':|:' + uv_set]
