...
class ASSET_TRACKER(object):
    ...
    def group_and_shape_nodes(self):
        camera_shapes = cmds.ls(type = 'camera')
        camera_shape_transforms = []
        for camera in camera_shapes:
            parent_of_camera_shape = cmds.listRelatives(camera,parent = True)
            camera_shape_transforms.append(parent_of_camera_shape[0])
        shape_nodes = cmds.ls(type = 'shape')
        self.shape_nodes_no_cameras = shape_nodes
        bad_shape_node_names = ['std_lgt_core_Shape','polySurface','perspShape','annotationShape']
        ...
        while i < size_shape_nodes:
            ...
            if shape_node in camera_shapes or shape_node in bad_shape_node_names:
                self.shape_nodes_no_cameras.remove(shape_node)
            shape_node_split = shape_node.split('_')
            shape_node_split_size = len(shape_node_split)
            if shape_node_split_size > 1:
                if shape_node_split[1] in bad_shape_node_names:
                    self.shape_nodes_no_cameras.remove(shape_node)
            i = i + 1
