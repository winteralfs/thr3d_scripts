...
#---------- procedural tools and data gathering methods ----------
    ...
    def gather_UV_displacement_links(self):
        ...
        #valid_material_types = ['VRayMtl','phong,blinn','lambert','VRayBlendMtl']
        valid_material_types = ['VRayMtl','phong,blinn','lambert']
        for material in materials:
            print ' '
            print 'material = ',material
            material_node_type = cmds.nodeType(material)
            print 'material_node_type = ',material_node_type
            print valid_material_types
            if material_node_type in valid_material_types:
                print '!! material is in valid types !!'
                material = material
                material_node_type = material_node_type
            else:
                print material + ' not valid, checking a connection'
                connections_0 = cmds.listConnections(material, destination = False) or []
                for connection in connections_0:
                    print '0 connection = ',connection
                    connection_type = cmds.nodeType(connection)
                    print '0 connection_type = ',connection_type
                    if connection_type in valid_material_types:
                        material = connection
                        material_node_type = connection_type
                    else:
                        print material + '1 not valid, checking a connection'
                        connections_1 = cmds.listConnections(connection, destination = False) or []
                        for connection in connections_1:
                            print '1 connection = ',connection
                            connection_type = cmds.nodeType(connection)
                            print '1 connection_type = ',connection_type
                            if connection_type in valid_material_types:
                                material = connection
                                material_node_type = connection_type
                            else:
                                print material + '2 not valid, checking a connection'
                                connections_2 = cmds.listConnections(connection, destination = False) or []
                                for connection in connections_2:
                                    print '2 connection = ',connection
                                    connection_type = cmds.nodeType(connection)
                                    print '2 connection_type = ',connection_type
                                    if connection_type in valid_material_types:
                                        material = connection
                                        material_node_type = connection_type
            print 'final material = ',material
            print 'final material node type = ',material_node_type
            if material_node_type == 'lambert' or material_node_type == 'blinn' or material_node_type == 'phong':
                shader_attr = '.transparency'
            if material_node_type == 'VRayMtl':
                shader_attr = '.opacityMap'
            if material_node_type == 'VRayBlendMtl':
            ...
            for linked_object in linked_objects:
                ...
                for tex in self.displacement_texture_object_dic:
                    ...
                    if obj == linked_object_compare:
                        print 'XX'
                        print 'connecting ' + tex + '.outColor' + ' to ' + material + shader_attr
                        print 'XX'
